...
export async function runWorkflowKernel(definition, input, deps = {}) {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input) || typeof input.task !== "string" || input.task.trim() === "") {
    return { ok: false, status: "refused", code: "kernel-input-invalid" };
  }
  ...
  const executeOne = async (agent, nodeId, instanceId, local = {}) => {
    if (active?.node_id === nodeId && Object.hasOwn(active.completed, instanceId)) {
      ...
      return structuredClone(active.completed[instanceId]);
    }
    ...
    const perform = async () => {
      ...
      if (mutating) {
        ...
        catch { return settled("failed", { code: "kernel-workspace-begin-failed" }); }
        if (tx?.ok === false) return settled("refused", { code: safeCode(tx.code, "kernel-workspace-begin-failed") });
      }
      ...
      if (mutating) {
        if (result.status === "ok") {
          ...
          if (!committed?.ok || !/^sha256:[0-9a-f]{64}$/.test(committed.workspace_ref ?? "")) {
            ...
            await deps.workspace?.rollback?.(tx);
          } else workspaceRef = committed.workspace_ref;
        } else await deps.workspace?.rollback?.(tx);
      }
      ...
      if (!accounted.ok && result.status === "ok") result = settled("failed", { code: accounted.code });
      ...
      if (!committed.ok) result = settled("failed", { code: committed.code });
      ...
      if (active?.node_id === nodeId) {
        active.completed[instanceId] = structuredClone(result);
        const saved = await checkpoint();
        if (!saved.ok) return settled("failed", { code: saved.code });
      }
      return result;
    };
  ...
  const executeWithRetry = async (agent, nodeId, instanceId, local = {}) => {
    for (let attempt = 1; attempt <= agent.retry.max_attempts; attempt += 1) {
      const result = await executeOne(agent, nodeId, `${instanceId}:attempt-${attempt}`, { ...local, attempt });
  ...
  const runInlineList = async (agents, nodeId, { concurrency = 1, failure = "abort", allow_failure_codes = [], local = {} } = {}) => {
    const values = await mapConcurrent(agents, concurrency, (agent, index) => executeWithRetry(agent, nodeId, `${nodeId}:${index}`, { ...local, index }));
  ...
  try {
    ...
    while (!cancelled) {
      ...
      let nodeStatus = "ok";
      if (node.kind === "agent") {
        const result = await executeWithRetry(node, current, `${current}:${visits[current]}`);
        ...
        for (let index = 0; index < node.stages.length; index += 1) {
          ...
          const result = await executeWithRetry(agent, current, `${current}:${visits[current]}:${index}`, { upstream });
        ...
        if (node.artifact && typeof deps.verifyArtifact === "function") {
          const artifact = await deps.verifyArtifact(node.artifact, {
        ...
        const values = await mapConcurrent(resolved.value, definition.limits.max_concurrency,
          (item, index) => executeWithRetry(node.body, current, `${current}:${index}`, { item, index }));
        const hardFailure = values.find((entry) => entry.status !== "ok"
        ...
        if (selected?.loop && deps.loops === false && node.loops_off) next = node.loops_off;
        else next = selected?.target ?? node.default;
        ...
        const result = await deps.checkpoint?.({ run_id: runId, node_id: current, reason: node.reason, outputs: structuredClone(outputs) });
        ...
        const child = await deps.resolveSubworkflow(node.workflow_id, node.version);
