...
The first Prime **code** slice after M0a: the Phase-1 safety substrate and the Phase-2
...
| --- | --- | --- | --- |
| **yolo-fence** | Pi extension (`tool_call` + `user_bash`) | `extensions/prime-fence.ts`, `extensions/lib/fence-rules.mjs` | `tests/fence-rules.test.mjs` (5), `tests/fence-extension.test.mjs` (10), loads in real Pi. See [`yolo-fence.md`](./yolo-fence.md). |
| **Worktree manager** | shell | `tools/worktree/prime-worktree.sh` | `tools/worktree/selftest.sh` (9) |
| **Objective-gate loop** | shell | `tools/loop/objective-gate-loop.sh` | `tools/loop/selftest.sh` (8) |
| **`\answer` resolver** | Pi extension (`registerTool` + `ctx.ui.select`) | `extensions/prime-answer.ts`, `extensions/lib/answer-core.mjs` | `tests/answer-core.test.mjs` (7) + `tests/answer-extension.test.mjs` (2 fake-Pi registration/execute tests) |
...
`.pi/settings.json` lists `extensions: ["../extensions/prime-fence.ts",
"../extensions/prime-answer.ts"]` (paths relative to `.pi/`), and `package.json` mirrors
...
- **No paid model calls** were made building or testing this. The extensions' interactive
  ...
  inventory does not prove `prime-ui` package-skill visibility; see
