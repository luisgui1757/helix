...
tool > slash command). Before prime adds *any* command, M0a records the baseline so
future growth is measured against it.

## Baseline captured in M0a

### Prime-added baseline: **zero** (2026-07-03)

At the original M0a inventory point, `prime` shipped **no** extensions, skills, prompt
...
### Current Prime-owned package surface: **one skill + one slash command** (2026-07-08)

Prime now ships a resource package with exactly one Prime-owned skill:
`skills/prime-ui/SKILL.md`, registered as `/skill:prime-ui` when skill commands are
enabled. Stage 3O PR1 adds exactly one extension slash command, `/prime`, as the
single Pi-native control surface over dashboard, no-live run preflight,
model/chain/profile views, and structural run list/status/prune. Prime ships no
...
and `~/.agents/skills` independently of the Prime package. A project package can keep
Prime's own surface to one skill; it cannot hide user-global skills that are loaded by
...
- **`enableSkillCommands` default = `true`** (`docs/settings.md:242`;
  ...
  `/skill:<name>` and **count against the `/` budget** the moment prime ships one. The
...
- **Static gating:** `--tools/-t`, `--exclude-tools/-xt`, `--no-builtin-tools/-nbt`,
  ...
  (experimental, `docs/skills.md:148`). Prime's plan-mode read-only restriction and the
  yolo-fence lean on these, not on disabling tools wholesale.

## The command-surface budget (target, from §6)

The roadmap's earlier target prime-added `/` set was **~8–9** clearly-named commands,
...
> Prime-owned skill command `/skill:prime-ui`.

Stage 3O supersedes the split control-surface plan with one command: `/prime`
and argument-completed verbs. Do not add `/prime-run`, `/prime-runs`,
`/prime-models`, `/prime-chains`, `/prime-profiles`, `/prime-worktrees`, or
`/prime-resources` as top-level slash commands. Everything else lands as behavior
...
Prime-added count as **0**; the current Prime-owned package count is **1 skill
...
`/prime` is justified as the first Pi-native UX layer over existing Stage 3
