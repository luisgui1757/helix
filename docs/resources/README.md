...
Status: Prime resource package plus Stage 3P whole-repo gap closure notes, 2026-07-09.

Prime now ships the project-owned Pi resource surface directly in this repo:

- One skill: `prime-ui`
- Three themes: `prime-rose-pine`, `prime-rose-pine-moon`,
  `prime-rose-pine-dawn`
- Three extensions: `prime-fence`, `prime-answer`, and `prime-command`
...
`prime-command` registers exactly one extension slash command, `/prime`. The
argument surface is `/prime`, `/prime help`, `/prime run [config-id]`, `/prime runs
list|status <run-id>|prune <run-id>`, `/prime models`, `/prime chains`, and
`/prime profiles`; it does not add `/prime-run`, `/prime-runs`,
`/prime-models`, `/prime-chains`, `/prime-profiles`, `/prime-worktrees`, or
`/prime-resources`.

## Skill Policy

`prime-ui` replaces the previous local fanout of many design-oriented skills for
Prime work. It is a single consolidated UI workflow covering shaping, building,
...
A project package can make Prime contribute exactly one skill, but it cannot
hide skills owned by the user's global Pi or agent configuration. If Prime later
...
`skill:prime-ui` from this project package or from explicit `--skill` loads. Treat
the static manifest/settings check as the supported no-auth proof for now; do not
claim `prime-ui` model visibility in headless `-p`/RPC until Pi exposes a reliable
...
--no-session` plus RPC `get_commands` listed `prime`. This proves the extension
slash command is discoverable in the no-auth/no-provider command inventory; it
does not change the older package-skill caveat above for `skill:prime-ui`.

## Theme Policy

Pi detects light or dark terminal background mode, but it does not import the
terminal emulator's full color palette. Prime therefore ships explicit Rose Pine
theme files and selects `prime-rose-pine` in `.pi/settings.json`.

The theme files are vendored from the audited `pi-themes-rose-pine@0.1.0`
tarball with Prime-prefixed names to avoid colliding with the upstream package's
...
uses a 60s default timeout for cold Pi startup and has re-confirmed `prime`
